...
from scr.agents.agent_01.multiAgent import run_multi_agent
from scr.agents.agent_01.multiAgent import StructuredOutput
...
class Eval_pipeline:
    def __init__(self, dataset: Dataset, log_folder_path: string = None):
        ...
        if log_folder_path is None:
            self.log_folder_path: string = os.path.join(
                os.path.dirname(
    ...
    def run_eval(self):
        ...
        for sample in tqdm(self.dataset):
            ...
            result = self.call_agent(question=sample.get("Question"))
    ...
    def call_agent(self, question: str) -> dict[str, Any] | None:
        answer = run_multi_agent(problem=question)

        return answer.get("structured_output")

    def save_logs(
        self, logs_list: list[dict[str, Any]], logs_list_formatted: list[dict[str, Any]]
    ) -> None:
